...
int main()
{
    // A raw vatiable that can be modified
    std::cout << std::endl;
    ...
    std::cout << "p_number : " << p_number1 << std::endl; // Address
    std::cout << "*p_number : " << *p_number1 << std::endl; // 23
    ...
    *p_number1 = 432;
    ...
    std::cout << "Pointer is modifiable, pointed value is constant : " << std::endl;
    int number3 {632}; // Although we can omit the const on number3 here and it is sill going to compile
                       // it is advised to be as explicit as possible in our code and put the const in front
                       // Make our intents CLEAR.
    ...
    // p_protected_var is a pointer to const data, we can't modify data through this pointer
    // regardless of whether the data itself is declared const or not.
    const int* p_protected_var {&protected_var};

    // *p_protected_var = 55; // compile error
    protected_var = 66;
    ...
    // We can't set up a modifiable pointer to const data though, we will get a compiler error
    // Invalid convertion from 'const type*' to 'type*'.
    const int some_data{55};
    // int * p_some_data {&some_data}; // Compiler error.
    // *p_some_data = 66;

    // Both pointer and pointed value are constant
    const int number5 {459};
    const int* const p_number5{&number5};
    std::cout << "Pointer is constant, value pointed is constant : " << std::endl;
    ...
    int number6 {333};
    // p_numer5 = &number6; // Error: Trying to assign to read only location
    std::cout << std::endl;

    // Pointer is constant (can't make it point anywhere else) but pointed value can change
    ...
    std::cout << "*p_number : " << *p_number7 << std::endl;
