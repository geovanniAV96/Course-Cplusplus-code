...
int main()
{
    ...
    //Concatenating string literals : Compiler Error
    //String literals are expanded into const char* arrays and C++ doesn't know how to add arrays with the + operator.
    //Hence the compiler error.

    std::cout << std::endl;
    std::cout << "Concatenating string literals : Compiler Error " << std::endl;
    ...
    std::cout << "str : " << str9 << std::endl;

    //std::string is a compound type, it has properties and behaviors one of the behaviors is append.
    //We access the behaviors using the . operator after the std::string variable name
    std::cout << std::endl;
    ...
    std::string str15{str14.append(5, '?')}; //Appends 5 '?' characteres
    std::cout << "str15 : " << str15 << std::endl;

    std::string str16{"The world is our shared home."};
    std::string str17{"Hello "};
    std::string str18{str17.append(str16, 4, 5)}; //Append a sub-section of another string starting from index 4, 5 characteres
    ...
    str27 += std::to_string(67.5f); // Treats 67 as a character code
                                    ...
                                    //std::string. But if we want the 67 appended to std::string
                                    //literally
