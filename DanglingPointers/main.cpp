...
int main()
{
    ...
    // p_number4 points to deleted memory. Dereferencing it will lead to undfined behavior : Crash / garbage or whatever
    std::cout << "p_number4 (after deleting p_number3) - " << p_number4 << " - " << *p_number4 << std::endl;

    // Solution 1 : Initialize our pointers immediately upon declaration
    std::cout << std::endl;
    ...
    // Make sure there is on clear pointer (master pointer) that owns the memory (responsible for releasing when necessary)
    // other pointers should only be able to dereference when the master pointer is valid

    std::cout << std::endl;
    std::cout << "Solution 3 : " << std::endl;
    int * p_number8 {new int{38}}; // Let's say p_number8 is the master pointer
