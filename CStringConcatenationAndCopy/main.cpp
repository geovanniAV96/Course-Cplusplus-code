...
int main()
{
    ...
    //source is going to be appended to dest, and the process will start by overriding the null character in dest
    ...
    //You can even use the returned pointer inmediately for print out
    //This is a pattern that we'll see a lot in C++ code out there.
    std::cout << std::strncat(dest2, source2, 6) << std::endl;

    //Or we can do std::strncat separately and print dest2
    std::strncat(dest2, source2, 6);
    std::cout << "The concatenated string is : " << dest2 << std::endl;

    //std::strcpy - signatur : char* strcpy(char* dest, const char* src);
    std::cout << std::endl;
