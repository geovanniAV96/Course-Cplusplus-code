...
int main()
{
    ...
    // We can use loops to print these things out easier
    p_score = scores;
    std::cout << std::endl;
    ...
    {
        std::cout << "Value : " << *(scores + i - 1) << std::endl; // Here we do the -1 thing, because scores is 
                                                                   // already pointing to the first element in the array
    }
    ...
    for(size_t i{std::size(scores)}; i > 0; --i)
    {
        std::cout << "Value : " << *(p_score--) << std::endl; // Here we do the -1 thing, because scores is 
                                                              // already pointing to the first element in the array
    }
    ...
    *(p_score + 2) = 33; // Equivalent to scores[2] = 33. Pointer arithmetic on p_score pointer
    ...
    std::cout << std::endl;
    std::cout << "Pointing out the array affter modification of 3 first elements: " << std::endl;
    for(size_t i{0}; i < std::size(scores); ++i)
    {
        std::cout << "Value : " << scores[i] << std::endl;
    }
